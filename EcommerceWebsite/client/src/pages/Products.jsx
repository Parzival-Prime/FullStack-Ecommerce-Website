...
function Products() {
    ...
    const addItemToCart = async (e) => {
        e.stopPropagation()
        const currentItemId = e.currentTarget.value
        const user = JSON.parse(localStorage.getItem('user'))

        let quantity;
        if (user && user.cart) {
            const cartItemIndex = user.cart.findIndex(item => item.productId === currentItemId)
            if (cartItemIndex !== -1) {
                quantity = user.cart[cartItemIndex].quantity += 1
            }
        }
        try {
            console.log({ productId: currentItemId, quantity })
            const { data } = await axiosInstance.post('/api/v1/auth/add-to-cart', { productId: currentItemId, quantity })
            if (data?.success) {
                        ...
                        >
                            {product ? (<Card sx={{ maxWidth: 345, height: 330, position: 'relative' }} productid={product._id} onClick={openProductPage}>
                                ...
                                <CardActions sx={{ display: 'flex', justifyContent: 'end', position: 'sticky', bottom: '0' }}>
                                    ...
                                    <IconButton onClick={(e) => addItemToCart(e)} sx={{
